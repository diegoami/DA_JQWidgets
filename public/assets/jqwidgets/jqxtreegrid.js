(function ($) {
    ...
    $.extend($.jqx._jqxTreeGrid.prototype, {
        defineInstance: function () {
            ...
            if (this === $.jqx._jqxTreeGrid.prototype) {
                return settings;
            }
        ...
        propertyChangedHandler: function (object, key, oldvalue, value) {
            ...
            if (key == "pageSizeMode" || key == "hierarchicalCheckboxes") {
                ...
                if (key == "filterable") {
                    ...
                    that.host.height(object.height);
                    that.host.width(object.width);
                    that._updatesize(false, true);
                }
                else if (key === "width") {
                    that.host.height(object.height);
                    that.host.width(object.width);
                    that._updatesize(true, false);
                    ...
                    that.content.css('left', '');
                    object.columns = object._columns;
                    that.vScrollBar.jqxScrollBar({ rtl: value });
                    that.hScrollBar.jqxScrollBar({ rtl: value });
                    that._render();
                }
                    ...
                    if (pagenum !== that.dataview.pagenum || parseInt(value) !== parseInt(oldvalue)) {
                        object._raiseEvent('pageSizeChanged', { pagenum: value, oldpageSize: oldvalue, pageSize: that.dataview.pagesize });
                    ...
                    that.selectionMode = value.toLowerCase();
                }
                else if (key == "touchmode") {
                    that.touchDevice = null;
                    that._removeHandlers();
                    that.touchDevice = null;
                    that.vScrollBar.jqxScrollBar({ touchMode: value });
                    that.hScrollBar.jqxScrollBar({ touchMode: value });
                    ...
                    if (value) {
                        that.base.host.addClass(this.toThemeProperty('jqx-fill-state-disabled'));
                    }
                    else {
                        that.base.host.removeClass(this.toThemeProperty('jqx-fill-state-disabled'));
                    }

                    if (that.pageable) {
                        if (that["pagernexttop"]) {
                            that["pagernexttop"].jqxButton({ disabled: value });
                            that["pagerprevioustop"].jqxButton({ disabled: value });
                            that["pagernextbottom"].jqxButton({ disabled: value });
                            that["pagerpreviousbottom"].jqxButton({ disabled: value });
                            that["pagerfirsttop"].jqxButton({ disabled: value });
                            that["pagerfirstbottom"].jqxButton({ disabled: value });
                            that["pagerlasttop"].jqxButton({ disabled: value });
                            that["pagerlastbottom"].jqxButton({ disabled: value });
                        ...
                        that.base.host.find('.jqx-grid-pager-number').css('cursor', value ? 'default' : 'pointer');
                    }
                    that.base.host.find('.jqx-grid-group-collapse').css('cursor', value ? 'default' : 'pointer');
                    that.base.host.find('.jqx-grid-group-expand').css('cursor', value ? 'default' : 'pointer');
                }
                else if (key == 'columnsHeight') {
                    that._render();
                }
                else if (key == 'theme') {
                    $.jqx.utilities.setTheme(oldvalue, value, that.base.host);
                    that.vScrollBar.jqxScrollBar({ theme: that.theme });
                    that.hScrollBar.jqxScrollBar({ theme: that.theme });
                    if (that.pageable && that["pagernexttop"]) {
                        that["pagernexttop"].jqxButton({ theme: that.theme });
                        that["pagerprevioustop"].jqxButton({ theme: that.theme });
                        that["pagernextbottom"].jqxButton({ theme: that.theme });
                        that["pagerpreviousbottom"].jqxButton({ theme: that.theme });
                        that["pagerfirsttop"].jqxButton({ theme: that.theme });
                        that["pagerfirstbottom"].jqxButton({ theme: that.theme });
                        that["pagerlasttop"].jqxButton({ theme: that.theme });
                        that["pagerlastbottom"].jqxButton({ theme: that.theme });
                        if (that["pagershowrowscombotop"].jqxDropDownList) {
                            if (that.pagerMode == "advanced") {
                                that["pagershowrowscombotop"].jqxDropDownList({ theme: that.theme });
                                that["pagershowrowscombobottom"].jqxDropDownList({ theme: that.theme });
                            }
                        }
                    }
                    if (that.filterable) {
                        ...
                        if (filters.length > 0) {
                            filters.jqxDropDownList({ theme: that.theme });
                        }
                        if (that.filtercolumnsList) {
                            that.filtercolumnsList.jqxDropDownList({ theme: that.theme });
                        }
                    }
